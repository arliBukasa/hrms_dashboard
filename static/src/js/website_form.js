odoo.define('hrms_dashboard.animation', function (require) {
'use strict';
...
sAnimation.registry.form_builder_send = sAnimation.Class.extend({
    ...
    send: function (e) {
        ...
        console.log("ici c'est l'envoi")
        var self = this;
        var form = this.$target.find('#my_form');
        if (form !== null){
                console.log(form.context.elements);
                for (var i=0;i<form.context.elements.length;i++){
                   var champ = form.context.elements[i];
                   var champ_oblig=["partner_last_name","partner_name","date_of_birth","partner_phone","adresse"];

                   if (champ.value == "") {
                       if (champ_oblig.indexOf(champ.name) !== -1){
                        e.preventDefault();
                        alert("veuillez actualiser la page remplir le champ: '"+champ.name+" '");
                        this.$target.find('.o_website_form_send').off().removeClass('disabled'); 
                        self.$target[0].reload(true);
                        return false;
                       }
                    }
